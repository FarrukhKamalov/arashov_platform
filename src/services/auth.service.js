  ...
  try {
    ...
    transporter
      .sendMail({
        ...
        to: email,
        subject: "Send OTP",
        html: `<body style="background-color: #3300">
...
const RegisterService = async (req, res) => {
  try {
    ...
    if (emailValidate)
      return res.json({ success: false, data: "Bunday foydalanuvchi bor" });
...
const LoginService = async (req, res) => {
  try {
    ...
    const emailValidate = await UserModel.findOne({ email: email });
    if (!emailValidate)
      return res.json({ success: false, data: "Bunday foydalanuvchi yoq" });
